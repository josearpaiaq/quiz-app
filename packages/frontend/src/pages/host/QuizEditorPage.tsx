...
export function QuizEditorPage() {
  ...
  const { data: quiz } = useQuery<QuizEntity & { questions: (QuestionEntity & { answers: AnswerEntity[] })[] }>({
    queryKey: ['quiz', id],
  ...
  if (!quiz) return <div className="min-h-screen bg-gray-950 text-gray-400 flex items-center justify-center">Loading…</div>;

  return (
    <div className="min-h-screen bg-gray-950 text-white flex flex-col">
      {/* Header */}
      <div className="flex items-center gap-4 px-6 py-4 border-b border-gray-800">
        <button onClick={() => navigate('/host')} className="text-gray-400 hover:text-white">←</button>
        <input
          className="bg-transparent text-xl font-bold outline-none flex-1 placeholder-gray-600"
          defaultValue={quiz.title}
          onBlur={(e) => updateQuiz.mutate({ title: e.target.value })}
        />
      </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6">
            <div className="space-y-3">
              <textarea
                ...
                rows={3}
                defaultValue={activeQ.text}
                onBlur={(e) => updateQuestion.mutate({ qId: activeQ.id, data: { text: e.target.value } })}
              />
              ...
              <div className="flex gap-4 flex-wrap">
                ...
                <select
                  ...
                  onChange={(e) => updateQuestion.mutate({ qId: activeQ.id, data: { timeLimit: Number(e.target.value) } })}
              ...
              {activeQ.answers.map((a) => (
                <div key={a.id} className="flex items-center gap-3 bg-gray-800 rounded-lg px-4 py-3">
                  <input
                    type="checkbox"
                    checked={a.isCorrect}
                    onChange={(e) => updateAnswer.mutate({ aId: a.id, data: { isCorrect: e.target.checked } })}
                    className="accent-green-500 w-4 h-4"
                  />
                  <input
                    className="flex-1 bg-transparent outline-none"
                    defaultValue={a.text}
                    onBlur={(e) => updateAnswer.mutate({ aId: a.id, data: { text: e.target.value } })}
                  />
                  <button onClick={() => deleteAnswer.mutate(a.id)} className="text-gray-600 hover:text-red-400 text-sm">✕</button>
                </div>
              ))}
              <button
